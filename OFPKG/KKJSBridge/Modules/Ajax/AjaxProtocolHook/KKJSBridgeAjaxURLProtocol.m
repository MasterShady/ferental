...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request {
    NSLog(@"%@ thread %@",request.URL, [NSThread currentThread]);
    // 看看是否已经处理过了，防止无限循环
    ...
    if ([request.URL.absoluteString containsString:kKKJSBridgeRequestId]) {
        //NSLog(@"~~ ajax canInitWithRequest yes:%@",request.URL);
        return YES;
    }
...
- (void)JSBridgeAjax:(id<KKJSBridgeAjaxDelegate>)ajax didReceiveResponse:(NSURLResponse *)response {
    ...
    //允许跨域.
    NSURLResponse *newResponse = [self appendRequestIdToResponseHeader:response];
