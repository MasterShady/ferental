...
class DeviceSmallCard : BaseView{
    private lazy var cover = UIImageView()
    private lazy var titleLabel: UILabel = {
        let label = UILabel()
        label.chain.font(.boldSystemFont(ofSize: 14)).text(color: .init(hexColor: "333333")).numberOfLines(0)
    ...
    override func configSubviews() {
        ...
        cover.chain.backgroundColor(.kExLightGray).contentMode(.scaleAspectFill)
...
class DeviceSmallCardCell : UICollectionViewCell{
    let cardView : DeviceSmallCard = .init(frame: .zero)
    var device: Device! {
        didSet{
    ...
    func configSubviews(){
        ...
        self.contentView.chain.corner(radius: 12).clipsToBounds(true)
        cardView.snp.makeConstraints { make in
            make.edges.equalToSuperview()
        }
    }
    
//    static func autoHight() -> CGFloat{
//        let cell = DeviceSmallCardCell(frame: .zero)
//    }
//
}
