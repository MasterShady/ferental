...
+(void)load{
    __block id observer = [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidFinishLaunchingNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
        Class cls = NSClassFromString(PKGContext.kWKBCCClassName);
        SEL sel = NSSelectorFromString(PKGContext.kRegisterProtocolSelectorName);
...
+ (void)prepareWebView{
    [[KKWebViewPool sharedInstance] makeWebViewConfiguration:^(WKWebViewConfiguration * _Nonnull configuration) {
        ...
        configuration.applicationNameForUserAgent = [NSString stringWithFormat:@"appId=%@ version=%@ topSafeInsets=%.0f bottomSafeInsets=%.0f",PKGManager.config.appId,PKGManager.config.appVersion,PKGContext.kStatusBarHeight,PKGContext.kBottomSafeInset];
    }];
...
- (void)commonInit{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didUpdateNotificationStatus) name:[PKGContext kNotificationStatusChanged] object:nil];
    ...
    _jsBridgeEngine.bridgeReadyCallback = ^(KKJSBridgeEngine * _Nonnull engine) {
        ...
        NSDictionary *data = @{
            @"NotificationAvailable": @(PKGManager.notificationAvaliable),
        };
...
- (void)didUpdateNotificationStatus{
    NSLog(@"%d",PKGManager.notificationAvaliable);
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
   ...
   if ( [urlString containsString:@"https://openapi.alipay.com"] && webView == _webView) {
         dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [request addValue:PKGManager.config.appURLScheme forHTTPHeaderField:@"Referer"];
    ...
    if ([navigationAction.request.URL.scheme isEqualToString:@"alipay"]) {
                ...
                NSString *afterHandleStr = [afterDecodeStr stringByReplacingOccurrencesOfString:@"alipays" withString:PKGManager.config.appURLScheme];
