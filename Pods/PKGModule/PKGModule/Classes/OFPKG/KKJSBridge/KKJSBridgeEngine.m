...
- (void)setupJSBridge {
    ...
    NSBundle *pkgBundle = [NSBundle bundleWithBundleName:@"PKGModule.bundle" podName:@"PKGModule"];
    NSString *bridgeJSString = [[NSString alloc] initWithContentsOfFile:[pkgBundle pathForResource:bridgeJSName ofType:@"js"] encoding:NSUTF8StringEncoding error:NULL];
    ...
    NSString *baseFunctionJSString = [[NSString alloc] initWithContentsOfFile:[pkgBundle pathForResource:@"BaseFunction" ofType:@"js"] encoding:NSUTF8StringEncoding error:NULL];
    WKUserScript *baseFunctionScript = [[WKUserScript alloc] initWithSource:baseFunctionJSString injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
    [self.webView.configuration.userContentController addUserScript:baseFunctionScript];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    NSLog(@"~~ messageHandler %@ content:%@",message.name,message.body);
